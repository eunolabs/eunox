...
You've read [the MCP post](./05-mcp-explained). You're convinced the protocol is the right abstraction — one server works with any client, the integration matrix collapses, your tools are composable. So you start wiring things up, and somewhere around the point where your agent successfully queries your production database and emails the results to a user, you notice something uncomfortable: there's nothing between the model and the backend except a JSON-RPC message.

 ...
 * |-----------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------|
 * | `env` (default)       | — none —                                                                                                                                                   |
 * | `azure-keyvault`      | `SECRET_STORE_AZURE_VAULT_URL` (required), `SECRET_STORE_AZURE_CREDENTIAL_TYPE`, `SECRET_STORE_AZURE_CLIENT_ID`, `SECRET_STORE_AZURE_CLIENT_SECRET`, `SECRET_STORE_AZURE_TENANT_ID` |
 * | `aws-secretsmanager`  | `SECRET_STORE_AWS_REGION` (falls back to `AWS_REGION`), `SECRET_STORE_AWS_ACCESS_KEY_ID`, `SECRET_STORE_AWS_SECRET_ACCESS_KEY`, `SECRET_STORE_AWS_SESSION_TOKEN` |
 * | `gcp-secretmanager`   | `SECRET_STORE_GCP_PROJECT_ID` (falls back to `GCP_PROJECT_ID`), `SECRET_STORE_GCP_KEY_FILE_PATH`                                                          |
 *
...
export function createSecretStoreFromEnv(
  ...
  switch (provider) {
    ...
    case 'azure-keyvault': {
      ...
      const credentialType = env['SECRET_STORE_AZURE_CREDENTIAL_TYPE'] as
        | 'default'
        | 'managed-identity'
        | 'client-secret'
        | undefined;

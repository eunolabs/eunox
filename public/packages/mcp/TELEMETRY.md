...
| `peakConcurrentSessions` | `number` | Peak number of MCP sessions that were active simultaneously during this invocation.  Always `1` for a stdio proxy.  For an HTTP proxy serving multiple clients concurrently this may be higher, providing a privacy-preserving signal of shared team usage.  No user identifiers, IPs, or hostnames are captured — only the peak count. |

...
| `MINTER_ISSUER_DID` | string | `did:web:minter.euno.local` | — | Minter's DID |
...
| `DEFENDER_ASSESSMENT_NAME_PREFIX` | string | `euno-agent-` | — | Assessment name prefix |
...
| `SECURITY_HUB_GENERATOR_ID` | string | `euno/posture-emitter/v1` | — | Finding generator ID |
...
(labelled `euno.dev/dev-only: 'true'`). These are **not suitable for production**.
...
| Helm (any K8s) | `k8s/helm/euno/` — umbrella chart |

...
Existing seams: pkg//src/runtime.ts (TokenVerifier, EvidenceSigner, KillSwitchManager), pkg//src/condition-registry.ts (CallCounterStore), pkg//src/manifest-validator.ts
Existing implementations to wire/reuse: internal/gateway/src/{verifier.ts,enforcement.ts,admin-api.ts,revocation-store.ts}, pkg/src/{call-counter-store.ts,redis-kill-switch.ts,redis-circuit-breaker.ts,ledger-signer.ts}, internal/issuer/ (for token issuance reference)
...
In pkg//src/cli.ts and the proxy bootstrap, add an enforcer config field ("local" | { url: string, apiKey: string }, default "local").
...
The verifier itself already lives in internal/gateway/src/verifier.ts. Confirm its TokenVerifier shape matches the seam in pkg//src/runtime.ts:423. If divergent, refactor the gateway to consume the seam from @euno/common-core (must not duplicate types per Critical Risks rule).
...
Use existing pkg/src/call-counter-store.ts (Redis impl) and redis-circuit-breaker.ts. Wire both into tool-gateway's enforcement path so rateLimit / maxCalls conditions are enforced across replicas.
...
Implement KmsEvidenceSigner in pkg/src/ with three drivers (Azure Key Vault, AWS KMS, GCP KMS) selected by config. Implement against the EvidenceSigner interface in pkg//src/runtime.ts:111.
...
Use existing pkg/src/redis-kill-switch.ts and its KillSwitchPersistenceBackend seam; implement the Postgres backend if not already present (the MVP commits to dual-write — verify and fill the gap).
...
New package: internal/minter/ (BSL, hosted-only initially per MVP line 646).
...
Single internal/gateway/Dockerfile already exists; produce a published image (BSL) with: configurable backends (Redis or in-memory for dev; Postgres or SQLite for dev; local HMAC or KMS).
...
Update README.md and pkg//README.md with the hosted option, keeping local-first messaging intact.
...
Lives in internal/integration-tests/ (existing package).

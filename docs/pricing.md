...
> This document is the authoritative reference for the Euno pricing tiers and
...
| | OSS | Self-Host | Cloud Free | Cloud Team | Cloud Enterprise |
|---|:---:|:---:|:---:|:---:|:---:|
| **License** | Apache-2.0 | BSL 1.1 | — | — | — |
| **Infrastructure** | BYO | BYO | Managed | Managed | Managed |
| **Price** | Free | Free | Free | Contact us | Contact us |
| **Agents** | Unlimited | Unlimited | Up to 5 | Unlimited | Unlimited |
| **Enforcement events / month** | Unlimited (local) | Unlimited | 50 000 | Unlimited | Unlimited |
| **Audit retention** | File only | You manage | 7 days | 90 days | Configurable |
| **SLA** | None | None | None | 99.9 % | 99.99 % |
| **Support** | Community | Community | Community | Email | Dedicated |
...
| Feature | OSS | Self-Host (BSL) | Cloud Free | Cloud Team | Cloud Enterprise |
|---|:---:|:---:|:---:|:---:|:---:|
| **Core enforcement** |||||
...
| `euno-mcp upgrade-to-hosted` CLI | — | — | ✅ | ✅ | ✅ |
| **Identity (Stage 4+)** |||||
...
### OSS — `@euno/mcp` (Apache-2.0)

The `@euno/mcp` package is Apache-2.0 and requires no server-side infrastructure.
All enforcement runs in-process inside each agent, using a local YAML policy file,
an in-memory call counter, and an optional HMAC audit log written to disk.

**When to use:** Individual developers, CI pipelines, local development, or any
use-case where shared state across agents is not required.

**How to get started:**

```bash
npm install @euno/mcp
```

See the [project README](../README.md) for quick-start instructions.
...
Fully managed Euno Cloud hosted on Euno infrastructure. Suitable for
...
**Getting started:** Sign up at [euno.dev](https://euno.dev) and run:

```bash
euno-mcp upgrade-to-hosted \
  --gateway-url https://gateway.euno.dev \
...
Contact us at [hello@euno.dev](mailto:hello@euno.dev) for pricing.
...
Self-hosters are not subject to metering or billing by Euno.

---

## 6. Upgrading between tiers

- **OSS → Cloud Free / Team / Enterprise:** Run `euno-mcp upgrade-to-hosted`
  (see [`docs/upgrade-to-hosted.md`](./upgrade-to-hosted.md)).
...
- **Cloud Free → Cloud Team / Enterprise:** Upgrade via the Euno Cloud console.
...
| Component | License |
|---|---|
| `@euno/mcp` (agent-side proxy, npm) | Apache-2.0 |

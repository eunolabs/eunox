...
(`enforcer: "local"`) to the hosted Euno gateway (`enforcer: "https://…"`).
...
1. **An Euno API key** (`sk-<prefix>.<secret>`) from the Euno Cloud console or
...
The `euno-mcp upgrade-to-hosted` command performs the three migration steps
...
curl -s \
  ...
  https://gateway.euno.example/api/v1/ping | jq .
...
GATEWAY_URL="https://gateway.euno.example"
ADMIN_KEY="<admin-api-key>"   # issued by Euno console or your admin
...
curl -s -X POST "${GATEWAY_URL}/admin/v1/policies" \
  ...
  -d "{
    \"policyId\": \"${POLICY_ID}\",
    \"manifest\": $(cat ./euno.policy.yaml | npx js-yaml)
  }" | jq .
# Expected: { "policyId": "…", "updatedKeys": 1, "capabilityCount": 3 }
```

> **Note:** `js-yaml` is a dev dependency of `@euno/mcp` and converts the
...
Open the file and find your `euno-mcp proxy` entry.  Change:
  ...
  "mcpServers": {
    "my-mcp-server": {
      ...
      "args": [
        ...
        "--enforcer-api-key", "sk-x7Kp9mRq.bL3nYv2w...",
        "--",
        "npx", "-y", "@modelcontextprotocol/server-filesystem", "/tmp"
      ]
    }
...
audit file (`~/.euno/audit.jsonl`) for any `GATEWAY_UNAVAILABLE` denials
...
|-------|-------------|-----|
| `API key is not valid` | Wrong `--api-key` or key revoked | Check the key in the Euno console |
| `HTTP 503 from /health` | Gateway unreachable | Check `--gateway-url` and network connectivity |
| `Admin key rejected` | Wrong `--admin-key` | Verify the admin key in the Euno console |
| `manifest validation failed` | Policy file has schema errors | Run `euno-mcp validate --policy <file>` to see details |

...
| Pri | Item                                                | What it unblocks                                                              | Hint where to start                                                             |
|-----|-----------------------------------------------------|-------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| 🟥  | **Dynamic policy engine (OPA / Cedar integration)**  | Manifests can reference policies expressed in OPA Rego or Cedar                | New adapter alongside the existing capability validators in `packages/common/src/capability-validators.ts`. Treat OPA/Cedar as a `CapabilityCondition` of `kind: 'policy'`. |
...
| Pri | Item                                                | What it unblocks                                                              | Hint where to start                                                             |
|-----|-----------------------------------------------------|-------------------------------------------------------------------------------|----------------------------------------------------------------------------------|
| 🟥  | **Continuous evidence-chain verification job**       | Catches log tampering automatically                                           | Schedule `node scripts/verify-evidence.js` over the previous day, alert on first failure. |

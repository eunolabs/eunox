...
| Kill-switch           | `<prefix>global`, `<prefix>killed_sessions` (SET), `<prefix>killed_agents` (SET); pub/sub channel `<prefix>events` (prefix = `KILL_SWITCH_KEY_PREFIX`, default `killswitch:`) | Sub-second via Redis pub/sub (`<prefix>events`); `KILL_SWITCH_REFRESH_INTERVAL_MS` (default 30 s) is the safety-net upper bound — see RedisKillSwitchManager. | Last known state is served from the per-process cache; a partition makes the kill-switch read-only until it heals. |

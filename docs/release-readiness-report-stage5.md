...
Stage 5 converts the euno platform from an enterprise-procurement conversation
...
| E6 | On-prem deployment bundle ships | ✅ | `k8s/helm/euno/` umbrella chart (6 services); `k8s/air-gap-images.txt`; `scripts/pull-air-gap-images.sh`; `docs/DEPLOYMENT.md` §Stage-5 |
...
| `@euno/common-core` | 70 | ✅ |
| `@euno/common-infra` | 186 | ✅ |
| `@euno/common` (compatibility shim + full suite) | 955 | ✅ |
| `@euno/capability-issuer` | 756 | ✅ |
| `@euno/tool-gateway` | 747 | ✅ |
| `@euno/agent-runtime` | 63 | ✅ |
| `@euno/db-token-service` | 14 | ✅ |
| `@euno/storage-grant-service` | 14 | ✅ |
| `@euno/posture-emitter` | 85 | ✅ (fixed — see §6) |
| `@euno/partner-issuer-sim` | 9 | ✅ (fixed — see §6) |
| `@euno/integration-tests` | 181 | ✅ |
| `@euno/cli` | 57 | ✅ |
| `@euno/mcp` | ≥700 (slow suite) | ✅ |
...
The `@euno/integration-tests` suite (181 assertions) covers:
...
### Defect 1 — `@euno/posture-emitter` test compilation failure

**Symptom:** Running `npm test -w @euno/posture-emitter` produced 15 TypeScript
compilation errors (`error TS2305: Module '"@euno/common"' has no exported
...
### Defect 2 — `@euno/partner-issuer-sim` test compilation failure
...
**Result:** All 9 `@euno/partner-issuer-sim` tests now pass.
...
`@euno/langchain` depends on `@euno/mcp` being built first and workspace
builds are unordered. This is benign: the full `npm run build` in `package.json`
explicitly pre-builds `@euno/common-core`, `@euno/common-infra`, `@euno/common`,
`@euno/posture-emitter`, and `@euno/mcp` in order before the final
...
*Generated by automated principal-architect review on 2026-05-20.*  
*Source: `docs/stage5executionplan.md`, `docs/mvp.md`, workspace test runs.*

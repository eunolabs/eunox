...
The following Euno secrets should be stored in GCP Secret Manager in
...
gcloud secrets create euno-prod-audit-ledger-hmac-secret \
  --project "${PROJECT_ID}" \
  --replication-policy automatic

echo -n "$(openssl rand -hex 32)" | \
  gcloud secrets versions add euno-prod-audit-ledger-hmac-secret \
  --project "${PROJECT_ID}" \
  --data-file=-

gcloud secrets create euno-prod-gateway-admin-api-key \
  --project "${PROJECT_ID}" \
  --replication-policy automatic

echo -n "$(openssl rand -base64 32 | tr -d '\n')" | \
  gcloud secrets versions add euno-prod-gateway-admin-api-key \
  --project "${PROJECT_ID}" \
  --data-file=-
...
for SECRET in \
  euno-prod-audit-ledger-hmac-secret \
  euno-prod-gateway-admin-api-key \
  euno-prod-partner-did-pin-secret \
  euno-prod-redis-url \
  euno-prod-audit-ledger-pg-url; do
...
for SECRET in \
  euno-prod-issuer-db-url \
  euno-prod-issuer-scim-bearer-token; do
...
```bash
gcloud secrets get-iam-policy euno-prod-audit-ledger-hmac-secret \
  --project "${PROJECT_ID}"
```
...
Create an ESO `SecretStore` in the `euno` namespace that uses the Workload
    ...
    gcpsm:
      ...
      auth:
        workloadIdentity:
          ...
          clusterName: euno-prod
          serviceAccountRef:
            name: euno-gateway   # the Workload Identity-annotated service account
```
...
spec:
  ...
  parameters:
    secrets: |
      - resourceName: "projects/my-gcp-project/secrets/euno-prod-audit-ledger-hmac-secret/versions/latest"
        fileName: "audit-ledger-hmac-secret"
      - resourceName: "projects/my-gcp-project/secrets/euno-prod-gateway-admin-api-key/versions/latest"
        fileName: "gateway-admin-api-key"
      - resourceName: "projects/my-gcp-project/secrets/euno-prod-partner-did-pin-secret/versions/latest"
        fileName: "partner-did-pin-secret"
      - resourceName: "projects/my-gcp-project/secrets/euno-prod-redis-url/versions/latest"
        fileName: "redis-url"
      - resourceName: "projects/my-gcp-project/secrets/euno-prod-audit-ledger-pg-url/versions/latest"
  ...
  volumes:
    - name: secrets-store
      csi:
        ...
        readOnly: true
        volumeAttributes:
          secretProviderClass: euno-gateway-spc
```
...
> `/mnt/secrets` is not read by the Euno process — only the synced Secret is.
...
gcloud secrets update euno-prod-audit-ledger-hmac-secret \
  --project "${PROJECT_ID}" \
  --add-topics "projects/${PROJECT_ID}/topics/euno-secret-rotation"
```
...
- [ ] Cloud Audit Logs alerts are configured for unexpected
      `secretmanager.googleapis.com/access` events on `euno-prod-*` secrets.

...
| ----- | ------------------------------------------------------------------ | ------------------------------------------------------------------------------------------- |
| F-1   | Per-user / per-token issuance rate limit (I-1) — **IMPLEMENTED** (tenant-aware, in-memory + Redis) | Token bucket keyed on `(tenantId, userId, agentId)` at the issuer. `IssuanceRateLimiter` interface in `@euno/common`; `InMemoryIssuanceRateLimiter` + `RedisIssuanceRateLimiter` implementations; `createIssuanceRateLimiterFromEnv` selects based on `REDIS_URL`. Wired into `CapabilityIssuerService.issueCapability` *before* signing. Denials surface as `RATE_LIMIT_EXCEEDED` (429) and increment `euno_issuer_issuance_rate_limit_denied_total{tenant,reason}`. Tenant-aware key shape is the load-bearing F-7 prerequisite (§6.1 #3). |
...
| F-3   | Response redaction obligation (I-3, R-4) — **IMPLEMENTED**         | Backed by R-4 step 1. `redactFields` strips fields on both `/api/v1/tools/invoke` and `/proxy/*`. |
...
| F-7   | Multi-region active/active issuer (backlog § 3) — **IMPLEMENTED** | `ISSUER_REGION` + `GATEWAY_REGION` env vars; optional `region` claim on capability tokens (preserved across attenuate/renew); `region` stamped on every `AuditLogEntry` and on the `euno.region` span attribute; surfaced on `/.well-known/capability-issuer`. Cross-region rate-limit coordination via the F-1 Redis backend (tenant-aware key shape was the load-bearing prerequisite). See `docs/MULTI_REGION_ISSUER.md` for topology, replication contract, and failover drill checklist. |

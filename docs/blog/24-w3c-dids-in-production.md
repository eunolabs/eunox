...
The negative cache TTL is significantly shorter than the positive cache TTL. We use `PARTNER_DID_NEGATIVE_CACHE_TTL_SECONDS` (default 60 seconds) for negative entries. The reasoning: a negative result might indicate a transient problem on the partner's side (they're deploying a new DID document) rather than a permanent state. We want to notice when the problem is fixed without having to wait 5 minutes. But we also don't want every bad token to trigger a fresh network call.
...
One design question that came up: should `did:key` be allowed in production partner registrations? The answer we landed on is no, with a configuration flag. If `ALLOW_DID_KEY_PARTNERS=true` is set, it's allowed (for development environments or early-stage partner integrations where the partner hasn't set up proper DID hosting yet). In production mode, `did:key` partner registration is rejected at proposal time with an explicit error message: "did:key partners are not supported in production mode; use did:web or did:ion."

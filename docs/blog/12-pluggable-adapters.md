...
```typescript
import { defaultSigningRegistry, SigningAdapter } from '@euno/capability-issuer/adapters';
...
defaultSigningRegistry.register('my-hsm', (config) => new HardwareHsmSigner(config));
```

...
> - [x] Task 5 — `euno request` and `euno validate-token` wired to live issuer: PKCE flow (`--idp-auth-url`, `--idp-token-url`, `--idp-client-id`), token persistence to `~/.euno/tokens/<agentId>.jwt` (0600), `--refresh` renewal path; top-level `euno validate-token` with JWKS verification + aud/iss enforcement; `euno revoke <jti>` via gateway admin API; `euno config set`; CLI tests pass; `docs/quickstart-stage-4.md` published
...
> - [x] Task 9 — `docs/self-host.md` Stage-4 section: §11 added with updated feature matrix (BYO-Issuer), issuer config reference, per-tenant IdP recipe (Cognito + Entra ID), template seed-data format, KMS-vs-local-key trade-off pointer to threat model §7, admin operator-JWT setup (Entra ID + Cognito + custom JWKS), full docker-compose additions, and Stage-4 security checklist
...
> - [x] Task 7 — `db-token-service` and `storage-grant-service` un-quarantined and promoted to GA (v1.0.0): `POST /api/v1/db-tokens` exchanges capability token for short-lived scoped DB credentials; `POST /api/v1/storage-grants` exchanges capability token for presigned URLs or SAS tokens; `DB_TOKENS_ENABLED` / `DB_INSTANCES_FILE` / `DB_USERNAME_POLICY_FILE`; `STORAGE_GRANTS_ENABLED`; both services added to docker-compose `full` profile; 12 integration tests in `db-token-service.test.ts` + 10 in `storage-grant-service.test.ts` (157 total integration tests); `STATUS.md` un-quarantined in both packages
...
> - [x] Task 14 — Stage-5 status block + reference materials: `docs/mvp.md` Stage-5 status block completed with all 15 task entries (Tasks 0–14); `README.md` §"Stage 5 — Enterprise" added (partner DID federation, cross-chain anchor, SOC2 audit export, AGT guard, SCIM, discovery v1.0.0, on-prem bundle); project status table updated to Stage 5 ✅ Done; `public/packages/cli/README.md` §"Stage 5: Enterprise Features" added (partner-federation CLI workflow, SOC2 audit export CLI commands, `euno audit export` and `euno partner` references); `CHANGELOG.md` added to all four un-quarantined packages (`partner-issuer-sim` updated, `posture-emitter`, `db-token-service`, `storage-grant-service` created at v1.0.0); `scripts/check-task14-references.mjs` CI lint + 15 tests in `scripts/__tests__/check-task14-references.test.mjs`

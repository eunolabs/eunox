...
- Both use dedicated non-root `euno` user/group
...
```yaml
container.apparmor.security.beta.kubernetes.io/<container-name>: localhost/euno-restricted
```
...
**Deployment:**
```bash
checkmodule -M -m -o euno_restricted.mod k8s/security-policies/selinux-policy.te
semodule_package -o euno_restricted.pp -m euno_restricted.mod
semodule -i euno_restricted.pp
```
...
1. **Kubernetes Cluster Setup:**
   ...
   kubectl label namespace euno-system pod-security.kubernetes.io/enforce=restricted
...
3. **Install SELinux (RHEL/CentOS nodes):**
   ...
   sudo semodule -i euno_restricted.pp
   ```
...
kubectl exec -n euno-system <pod-name> -- cat /proc/self/attr/current

# Check resource limits
kubectl describe pod -n euno-system <pod-name> | grep -A 10 "Limits"

# Check network policies
kubectl get networkpolicies -n euno-system

# Test forbidden operations (should fail)
kubectl exec -n euno-system <pod-name> -- mount /dev/sda1 /mnt  # Should fail
kubectl exec -n euno-system <pod-name> -- su -  # Should fail
```
...
# View denials
sudo ausearch -m avc -ts recent | grep euno

# Reload policy
sudo semodule -r euno_restricted
sudo semodule -i euno_restricted.pp
```
...
kubectl describe pod -n euno-system <pod-name> | grep -i throttl

# View resource quota usage
kubectl describe resourcequota -n euno-system
```

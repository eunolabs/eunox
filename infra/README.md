...
| ---------------------------- | --------------------- | -------------------------------------------------------------- |
| `REDIS_URL`                  | _(unset — in-memory)_ | Redis URL for all control-surface stores.                      |
| `AUDIT_LEDGER_BACKEND`       | `none`                | `none`, `in-memory`, `postgres`, `per-replica-postgres`, `acl` |
| `AUDIT_LEDGER_PG_URL`        | _(unset)_             | PostgreSQL DSN for the audit ledger.                           |
| `AUDIT_LEDGER_HMAC_SECRET`   | _(unset)_             | HMAC-SHA-256 secret for per-row ledger integrity.              |

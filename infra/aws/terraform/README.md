# Eunox AWS Terraform — Modular Layout

Modular Terraform configuration for the Eunox Capability-Native Agent Governance
...
terraform {
  backend "s3" {
    bucket         = "eunox-tf-state-prod"
    key            = "eunox/platform.tfstate"
    region         = "us-east-1"
    dynamodb_table = "eunox-tf-lock"
    encrypt        = true
  }
}
```
...
| `aws_eks_fargate_profile.eunox_system` | Fargate profile for `eunox-system` namespace   |
...
| Resource                                   | Description                                    |
| ------------------------------------------ | ---------------------------------------------- |
| `aws_cloudwatch_log_group.runtime`         | `/eunox/runtime` — application logs            |
| `aws_cloudwatch_log_group.audit`           | `/eunox/audit` — audit ledger entries          |
...
- Adds EKS Fargate profile for `eunox-system` namespace (`compute/`)
